...
//! supersample-in-motion switch and the grid size. The only environment
...
use iced_texture_cache::{Element, PixelSnap, Renderer, TextureCache, cached};
...
impl Benchmark {
    ...
    fn scene_panel(&self) -> Element<'_, Message> {
        ...
        let scene: Element<'_, Message> = if self.cached_on {
            cached(self.scene_cache.clone(), heavy_scene(cols, rows))
                .translate(offset)
